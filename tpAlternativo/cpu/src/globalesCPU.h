...
#include <sys/types.h>
#include <netdb.h>
#include "/home/utnso/git/ansisop-panel/panel/cpu.c"
...
#define KRED  "\x1B[31m"
#define KWHT  "\x1B[37m"
#define refcpuConfig "/home/utnso/git/tp-2014-2c-rafagadeterror/tpAlternativo/cpu/cpu.conf"
...
unsigned char* conversorIntByteCode(int num, unsigned long n, unsigned char* bytesnum){
...
t_registros_cpu* registrosPanel(tcb*tcb){
	t_registros_cpu* regs= malloc(sizeof(t_registros_cpu));
	regs->registros_programacion[0]=tcb->A;
	regs->registros_programacion[1]=tcb->B;
	regs->registros_programacion[2]=tcb->C;
	regs->registros_programacion[3]=tcb->D;
	regs->registros_programacion[4]=tcb->E;
	regs->K=tcb->kernelMode;
	regs->I=tcb->pid;
	regs->M=tcb->M;
	regs->P=tcb->P;
	regs->X=tcb->X;
...
//funcion para cpu
void cambiosRegistrosPanel(tcb*tcb){
	t_registros_cpu* cambios = registrosPanel(tcb);
	cambio_registros(*cambios);
	free(cambios);
}

//funcion para cpu
void comienzoEjecucionPanel(tcb* tcb, uint32_t quantum){
	t_hilo* tcbTransformado = transformarTCB(tcb);
	comienzo_ejecucion(tcbTransformado,quantum);
	free(tcbTransformado);
}
